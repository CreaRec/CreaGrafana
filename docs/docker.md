...
Upstream images: `grafana/alloy`, `grafana/loki`, `grafana/tempo`, `grafana/mimir`, `grafana/grafana` (tags pinned in `.env`).
...
```sh
mkdir -p /home/crearec/crea-grafana/data/{alloy,loki,tempo,mimir,grafana}
